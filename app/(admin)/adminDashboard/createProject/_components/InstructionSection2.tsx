...
import DialogBoxAnswer from './DialogBoxAnswer';

import { Instruction,Instruction1,Instruction2,PMF,ProjectData,Data } from '@/lib/interfaces';

interface InstructionSection2Props {
  instructions: Instruction2[];
  setInstructions: React.Dispatch<React.SetStateAction<Instruction2[]>>;
}

const InstructionSection2: React.FC<InstructionSection2Props> = ({ instructions, setInstructions }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [editingId, setEditingId] = useState<number | null>(null);

  const handleSave = (title: string, description: string, type: string, options: string[] = []) => {
    const updatedInstructions = editingId
      ? instructions.map((item) =>
          item.id === editingId ? { ...item, title, description, type, options } : item
        )
      : [...instructions, { id: Date.now(), title, description, type, options }];
  ...
  return (
    <div className="bg-white rounded-3xl shadow-md w-full flex flex-col justify-between h-full">
      <div className="mb-2 p-6">
        <h2 className="text-center text-xl font-semibold">Task Format</h2>
      ...
      <div className="flex-grow px-6 max-h-4/6 overflow-y-auto">
        {instructions.length === 0 ? (
          <p className="text-gray-500 text-center mt-4">No task format added yet.</p>
        ) : (
...
export default InstructionSection2;
